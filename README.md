...
- When automatic input switching is unreliable, each side now exposes a built-in "manual handoff assistant". The current owner can prepare its own local desktop/output state first, and you can then finish the transfer with the monitor's physical input button without desynchronizing app state.
...
- Switch success confirmation now checks local topology first for the current attempt. If Windows has already collapsed back to its fallback screen, or macOS has already lost the only shared display, the app treats that as a successful ownership transfer instead of waiting for LAN confirmation.
...
- When macOS hands the shared screen back to Windows and a uniquely discovered Windows peer is available, the macOS app now asks Windows to pre-warm the shared display path before sending DDC. This helps monitors that would otherwise fall back to the Mac input because Windows had already detached the shared screen from desktop topology.
...
- The settings page now also shows a "Samsung local private path" diagnostics card. It checks whether the current machine can see a plausible local Samsung control entry point through official software, USB evidence, or Smart Monitor network discovery before you spend more time on protocol reverse engineering.
