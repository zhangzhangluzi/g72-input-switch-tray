...
- Every Windows PowerShell helper runs through the bundled `hidden-process.exe` launcher using Win32 `CreateProcessW` with `CREATE_NO_WINDOW`, redirected pipes, no command shell, and explicit `-WindowStyle Hidden`, so background topology checks cannot delegate to a visible Windows Terminal or PowerShell window. The launcher is rebuilt from source during Windows packaging and does not bundle a .NET runtime.
