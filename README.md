...
- When automatic input switching is unreliable, each side now exposes a built-in "manual handoff assistant". The side that currently owns the picture prepares a local "transfer" step, the other side prepares a local "receive" step, and the app cancels stale manual sessions after 30 seconds so Windows can roll back any temporary desktop-topology changes.
