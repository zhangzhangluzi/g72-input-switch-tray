  ...
  return {
    ...
    submenu: [
      ...
      ...(peerTarget
        ? [
            {
              label: `交给${getPeerMachineLabel(peerTargetId, monitorContext.monitor)}（${peerTarget.label}）`,
...
async function handleWindowsTakeoverRequest(response, requestUrl, monitorId) {
  try {
    ...
    redirectToSettingsPage(response, requestUrl, {
      status: "error",
      message: normalizeText(error.message) || "Windows 接回共享屏失败。",
    });
  }
}
...
async function takeoverWindowsDetachedMonitorUnlocked(
  ...
  if (!takeoverContext) {
    throw new Error("没有找到可接回的 Windows 断开屏幕。");
  }
  ...
  try {
    ...
    await waitForWindowsTopologyDeviceAttachmentState(
      ...
      true,
      `${getMonitorDisplayTitle(monitorConfig)} 已发送接回命令，但 Windows 显示拓扑没有稳定保留这块屏。`,
      30000,
      3000
    );
    clearMonitorPendingRestore(monitorConfig.id);
...
function formatWindowsTakeoverError(monitorConfig, localTarget, error) {
  ...
  if (/Failed to attach display|重新加回桌面拓扑|bad mode|code -2/i.test(rawMessage)) {
    return `${monitorTitle} 无法加回 Windows 桌面拓扑。Windows 没有接受这块断开屏的显示模式，接回没有生效。原始错误：${rawMessage}`;
  }

  return rawMessage || "Windows 接回共享屏失败。";
}
...
async function getWindowsDetachedTakeoverContexts({
  ...
  for (const monitorConfig of getStoredMonitorConfigs()) {
    ...
    if (
      ...
      !isWindowsPhysicalDetachedDisplayCandidate(topologyDisplay)
    ) {
      continue;
    }
...
function isWindowsAutoGeneratedTakeoverConfig(monitorConfig) {
  if (!["待接回屏幕", "可接管屏幕"].includes(monitorConfig?.roleLabel)) {
    return false;
  }
...
function createWindowsDetachedTakeoverContext(monitorConfig, topologyDisplay, takeoverSource) {
  ...
  return {
    ...
    status: {
      ...
      ddcAvailable: false,
      ddcProbeStatus: "",
      ddcUnavailableReason: "这块屏当前不在 Windows 桌面里，接回时会先加回桌面。",
      currentInputValue: null,
      currentInputError: null,
    },
  };
...
function getUsageIntroText() {
  if (process.platform === "win32") {
    return "把共享屏当成一块可以交接的屏幕：Windows 有画面时用“交给对方机器”，共享屏在对方机器时用“接回 Windows 的共享屏”。DP/HDMI 数值只是高级校准。";
  }
  ...
  const items =
    process.platform === "win32"
      ? [
          {
            title: "交给对方",
            body: "共享屏现在显示 Windows 画面时，点“交给对方机器”。软件会切到配置的对方接口，并把这块屏从 Windows 桌面移除。",
          },
          {
            title: "接回 Windows",
            body: "共享屏现在在对方机器上时，点“接回 Windows 的共享屏”。软件会先把屏幕加回 Windows 桌面，再切回 Windows 接口。",
          },
          {
            title: "灰色第二屏",
...
function renderWindowsTakeoverSection(windowsTakeoverContexts) {
  ...
  if (contexts.length === 0) {
    return `<details class="advanced">
      <summary>接回 ${escapeHtml(getLocalMachineLabel())}：当前没有可接回共享屏</summary>
      <div class="help" style="margin-top: 12px;">如果共享屏已经在 ${escapeHtml(
        getLocalMachineLabel()
      )} 上，它会出现在下面的“当前在 ${escapeHtml(
        getLocalMachineLabel()
      )} 上的屏幕”里；如果它正在对方机器上，但 Windows 完全看不到断开的显示设备，这里也不会出现按钮，需要从对方机器切走或用显示器菜单。</div>
    </details>`;
  }
    ...
    <div class="daily-card">
      ...
      <form method="post" action="/api/${encodeURIComponent(state.controlToken)}/switch/${encodeURIComponent(
        ...
        <button type="submit"${directSwitchEnabled ? "" : " disabled"}>
          交给${escapeHtml(getPeerMachineLabel(peerTargetId, monitorContext.monitor))}（${escapeHtml(peerTarget.label)}）
        </button>
      </form>
      <div class="help" style="margin-top: 10px;">
...
function renderMonitorConfigForm(monitorContext) {
  ...
  return `<form method="post" action="/api/${encodeURIComponent(
    ...
    <div class="interface-grid">
      ${TARGET_IDS.map((targetId) => {
        const target = getTarget(targetId, monitorConfig);
        const defaultDeviceLabel = isLocalInterfaceTarget(targetId, monitorConfig)
          ? getLocalMachineLabel()
          : "对方机器";
        return `<div class="interface-card${isLocalInterfaceTarget(targetId, monitorConfig) ? " current" : ""}">
          ...
          <label>
            连接设备名（可选）
            <input name="${escapeHtml(targetId)}DeviceLabel" maxlength="40" value="${escapeHtml(
              target.deviceLabel
            )}" placeholder="${escapeHtml(defaultDeviceLabel)}">
          </label>
          <label>
            输入值
...
function renderSharedStyles() {
  return `
    :root {
      ...
      font-family: "PingFang SC", "Microsoft YaHei", "Noto Sans SC", "PingFang TC", "Microsoft JhengHei", sans-serif;
    }
    html {
      min-height: 100%;
    }
...
function cloneInterfacesConfig(sourceInterfaces = null) {
  ...
  for (const targetId of TARGET_IDS) {
    nextInterfaces[targetId] = {
      ...
      deviceLabel: normalizeText(sourceInterfaces?.[targetId]?.deviceLabel),
    };
  }
...
function getTarget(targetId, monitorConfig = null) {
  ...
  return {
    ...
    deviceLabel: normalizeText(configuredTarget.deviceLabel),
...
function getPeerMachineLabel(targetId = "", monitorConfig = null) {
  const configuredLabel = normalizeText(monitorConfig?.interfaces?.[targetId]?.deviceLabel);
  if (configuredLabel) {
    return configuredLabel;
  }

  return isLocalInterfaceTarget(targetId, monitorConfig) ? getLocalMachineLabel() : "对方机器";
}
...
function shouldUseWindowsPowerShellTemp(file) {
  return (
    process.platform === "win32" &&
...
function getWindowsPowerShellEnv() {
  const tempPath = getWindowsHelperTempPath();
  return tempPath
    ? {
        TEMP: tempPath,
        TMP: tempPath,
...
function getWindowsHelperTempPath() {
  const basePath = path.join(app.getPath("userData"), "tmp");
  try {
    fs.mkdirSync(basePath, { recursive: true });
    return basePath;
  } catch (error) {
    appendDiagnosticLog("Failed to create Windows helper temp directory", error);
    return "";
  }
}
